...
sound=SoundManager("C:/Users/ASUS/SteadySphere/data/sounds/sound.m4a")

pid_x=PID(Kp=0.2,Ki=0.004,Kd=0.07)
pid_y=PID(Kp=0.09999999999999996,Ki=0.007,Kd=0.03)

colorTracker=ColorTracker()
yellow=[0,255,255]#[45,115,205]
rectangle_color=[45,115,205]
blue=[177,118,18]
lower_yellow,upper_yellow=colorTracker.getRangeHSV(yellow)
lower_rectangle_color,upper_rectangle_color=colorTracker.getRangeHSV(rectangle_color)

maze=Maze()
lower_blue,upper_blue=colorTracker.getRangeHSV(blue)
wall_lower_color=np.array([0,0,0])
wall_upper_color=np.array([180,255,50])
path_lower_color=np.array([35,40,40])
path_upper_color=np.array([85,255,255])
...
class ProjectManager(QWidget):
    ...
    def __init__(self):
        ...
        pixmap=QPixmap("C:/Users/ASUS/SteadySphere/data/images/experience.png")
        ...
        pixmap=QPixmap("C:/Users/ASUS/SteadySphere/data/images/steadySphere.jpg")
        ...
        pixmap=QPixmap("C:/Users/ASUS/SteadySphere/data/images/medicine-ball.png")
    ...
    def run_pid_ball_balance(self,platform_center_locked=False,saved_platform_X=None,saved_platform_Y=None,ballCenter_X=None,ballCenter_Y=None):        
        ...
        while True:
            ...
            if not platform_center_locked:
                mask_clean,platform_X,platform_Y=colorTracker.track_rectangle_position(frame,HSV_frame,lower_rectangle_color,upper_rectangle_color)
            ...
            if key==ord('l'):
                ...
                platform_center_locked=False
                print("Platform tracking unlocked","INFO")
            print(f"Platform  center: X={saved_platform_X}, Y={saved_platform_Y}", "PLATFORM")
            
            mask_clean,ballCenter_X,ballCenter_Y=colorTracker.track_ball_position(frame,HSV_frame,lower_yellow,upper_yellow)
            print(f"Ball  center: X={ballCenter_X}, Y={ballCenter_Y}", "BALL")
            print("\n")
            ...
            if ballCenter_X is not None and saved_platform_X is not None:
                ...
                print(f"Error X={error_x}", "ERROR")
                print("\n")
                if abs(error_x)<5:
                    output_x=0
                    print("X Dead Zone-No correction needed.","INFO")
                else:
                    output_x=pid_x.PIDcompute(error_x)
                    print(f"pid x={output_x}", "INFO")

                angle_x=gimbalController_x.mapPIDtoAngle(output_x,-100,100,125,155)
                print(f"angle x={angle_x}", "INFO")
    ...
    def run_maze_game(self):
        ...
        while True:
            ...
            frame,HSV_frame=colorTracker.prepareFrameForTrackingByHSV(frame)
            maze_img,maze_rect,maze_mask=colorTracker.track_maze_border(frame,HSV_frame,lower_blue,upper_blue)
